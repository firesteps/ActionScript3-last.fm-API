package{
	...
	import fm.last.as3.LFMLibrary;
	import fm.last.as3.LFMSession;
	
	public class LFMLibraryExample extends Sprite{
		private var _session:ILFMSession;
		public function LFMLibraryExample()	{
			super();
			_session = new LFMSession('b25b959554ed76058ac220b7b2e0a026');
			var library:LFMLibrary = new LFMLibrary(_session);
			
			addListeners(library.getAlbums("joanofarctan"));
			addListeners(library.getArtists("joanofarctan"));
			addListeners(library.getTracks("joanofarctan"));
			// if you haven't authorized session, this requests will throw service errors 
			addListeners(library.addAlbum("Play", "Moby"));
			addListeners(library.addArtist("Moby"));
			addListeners(library.addTrack("Moby", "Run on"));
		}
